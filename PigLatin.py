""" RULES: Pig Latin is an English language game where the goal is to hide the meaning of a word from people not aware of the rules.
...
def pig_latin(s):
    ...
    vowel = "aeiouAEIOU"
    if any(char.isdigit() for char in s) or not s:
        return None
    
    for i,v in enumerate(s):
        if i==0 and v in vowel:
            return (s+"way").lower()
        elif v in vowel:
            return (s[i:] + s[:i] + "ay").lower()
